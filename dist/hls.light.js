...
(function (global, factory) {
  ...
  var QC_IMAGE_INF_REGEX = /RESOLUTION=(\d+x\d+),LAYOUT=(\d+x\d+),DURATION=([\d.]+),URI=([^\r\n]*)/;
    ...
    M3U8Parser.parseMasterPlaylist = function parseMasterPlaylist(string, baseurl) {
      ...
      while ((result = MASTER_PLAYLIST_REGEX.exec(string)) != null) {
        if (result[1]) {
          ...
          var qcImageInf = result[5];
          var qcMatch = QC_IMAGE_INF_REGEX.exec(qcImageInf);
          if (qcMatch) {
            var _resolution = qcMatch[1];
            var layout = qcMatch[2];
            var duration = parseFloat(qcMatch[3]);
            var _uri = qcMatch[4];
            parsed.qcImage = {
    ...
    _proto.checkRetry = function checkRetry(errorEvent) {
      ...
      if (retry) {
        ...
        if (isTimeout && (_errorEvent$context = errorEvent.context) != null && _errorEvent$context.deliveryDirectives) {
          var _errorEvent$context2;
          // The LL-HLS request already timed out so retry immediately
          this.warn("Retrying playlist loading " + (retryCount + 1) + "/" + retryConfig.maxNumRetry + " after \"" + errorDetails + "\" \"" + (retryCount ? 'without' : 'with') + "\" delivery-directives");
          this.loadPlaylist(retryCount ? undefined : (_errorEvent$context2 = errorEvent.context) == null ? void 0 : _errorEvent$context2.deliveryDirectives);
        } else {
